...
<head>
    ...
    // two string array to format the x-axis of the plots
    $mon_array = array("Jan.", "Feb.",
        "Mar.","Apr.","May",
        "June","July","Aug",
        "Sept.","Oct.","Nov.",
        "Dec.");
...
<table class='tg' style="width:50%">
    ...
    while($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)){
        $stid_get_rating = oci_parse($conn,'SELECT AVG(RATING) DISHRATE FROM G14ORDER
                            WHERE BUSSID = :target_id AND DISHID = :dish_id');
        oci_bind_by_name($stid_get_rating, ':target_id', $bussid);
        oci_bind_by_name($stid_get_rating, ':dish_id', $row["DISHID"]);
        oci_define_by_name($stid_get_rating,'DISHRATE', $dish_rate);
        oci_execute($stid_get_rating);
        oci_fetch($stid_get_rating);
        print "<tr>\n";
//        foreach ($row as $item) {
//            print "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
//        }
        print "    <td class='tg-175l'>" . ($row["ITEMNAME"] !== null ? htmlentities($row["ITEMNAME"], ENT_QUOTES) : "&nbsp;") . "</td>\n";
        print "    <td class='tg-175l'>" . ($dish_rate !== null ? htmlentities(number_format(floatval($dish_rate), 2, '.',''), ENT_QUOTES) : "&nbsp;") . "</td>\n";
        print "    <td class='tg-175l'>" . ($row["DISHNUM"] !== null ? htmlentities($row["DISHNUM"], ENT_QUOTES) : "&nbsp;") . "</td>\n";
        print "</tr>\n";
        oci_free_statement($stid_get_rating);
    }
    oci_free_statement($stid);
    ...
    $ind = 1;
    while($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {
        while(intval($row["Month"] != $ind)){
            $order_mon_dataSet->addPoint(new Point($mon_array[$ind-1], "0"));
            $ind++;
        }
        $order_mon_dataSet->addPoint(new Point($mon_array[intval($row["Month"])-1], $row["ORDERNUM"]));
        $ind++;
    }
    while($ind <= 12){
        $order_mon_dataSet->addPoint(new Point($mon_array[$ind-1], "0"));
    ...
    $ind = 0;
    while($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {
        while(intval($row["Hour"]) != $ind){
            $order_hr_dataSet->addPoint(new Point($hour_array[$ind], "0"));
            $ind++;
        }
        $order_hr_dataSet->addPoint(new Point($hour_array[intval($row["Hour"])], $row["ORDERNUM"]));
        $ind++;
    }
    while($ind < 24){
        $order_hr_dataSet->addPoint(new Point($hour_array[$ind], "0"));
    ...
    $temp_list = $order_hr_dataSet->getPointList();
    //var_dump($temp_list);
